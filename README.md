...
Informatics Engineering graduate with a strong interest in Data Science,
Machine Learning, and Deep Learning. Experienced in data cleaning,
exploratory data analysis, and building machine learning models to solve
data-driven business problems. Built a deep learning model based on the
ConvNeXt architecture for image classification. Comfortable working through
the full pipeline — from data preparation and analysis to modeling and
business recommendation.

📍 Samarinda, Indonesia
📫 henryhafid07@gmail.com
🔗 [LinkedIn](ganti-link-linkedin-kamu) · [CV](ganti-link-cv-kamu)
...
`Scikit-learn` `PyTorch` `TensorFlow` `ConvNeXt` `Regression` `Classification`
`Clustering` `Feature Engineering` `Model Evaluation`

**Computer Vision**
`Image Preprocessing` `Image Classification` `CNN`

**Data Visualization**
`Matplotlib` `Seaborn` `Plotly`

**Tools**
`Git` `GitHub` `Jupyter Notebook` `VS Code` `Excel` `Google Spreadsheet`
...
`Python` `Scikit-learn` `RFM` `K-Means` `PCA`
